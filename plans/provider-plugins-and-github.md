...
- `extract_env_vars(text: str) -> list[str]`
  - Extract explicit env var names from docs (e.g., `GITHUB_TOKEN`).
...
- `status: valid | invalid | unknown | unverifiable | error`
- `checked_at`
- `error_code` (optional)
...
- `unverifiable`: returned when a provider has no safe free/auth validation endpoint. See [`plans/key-validation.md`](plans/key-validation.md:35).
